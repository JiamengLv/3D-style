# 代码网站： https://www.codetd.com/article/14432951

#  1.如何将2维图像上的点映射到三维坐标上
#  2.如何生成点云数据
...
class point_cloud_generator():
    ...
    def __init__(self, rgb_file, depth_file, save_ply, camera_intrinsics=[784.0, 779.0, 649.0, 405.0]):
        ...
        self.rgb = cv2.imread(self.rgb_file)
        print((self.rgb.shape[0],self.rgb.shape[1]))

        # 如果可以提供正常的深度图

        depth = cv2.cvtColor(cv2.imread(self.depth_file,-1), cv2.COLOR_BGR2GRAY)
        print((depth.shape[0], depth.shape[1]))
        self.depth = 250-cv2.resize(depth, (self.rgb.shape[1],self.rgb.shape[0]))
        print((self.depth.shape[0], self.depth.shape[1]))
...
if __name__ == '__main__':
    ...
    camera_intrinsics = [1000, 1000, 250, 100]            # 相机角度以及位置

    # 关键是相机的角度以及位置 如何设置  ？？？？？

    rgb_file = "./show_image/Image.jpg"                                                         # 颜色图像
    depth_file = "./out_show_image/out_image.jpg"                                                       # 深度图像
    save_ply = "data.ply"                                                           #
