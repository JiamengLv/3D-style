...
# 权重标准化的卷积层，没有进行权重标准化或导致梯度消失、权重为0
...
class deepFeatureExtractor_ResNext101(nn.Module):
    ...
    def forward(self, x):
        ...
        return out_featList               # 返回多层特征提取的结构
...
class Lap_decoder_lv5(nn.Module):
    ...
    def forward(self, x, rgb):
        ...
        dec2 = self.decoder2_reduc1(torch.cat([dec2,cat3],dim=1))     # 特征拼接
        ...
        # fit laplacian image range (-80,80), depth image range(0,80)  扩大取值范围
...
class LDRN(nn.Module):
    ...
    def forward(self, x):
        ...
        #### 特征边界提取： 类似特征金子塔，对每一层的数据进行下采样以及上采样操作
        # 下采样
